...
export async function fetchPage(url) {
    try {
        ...
        console.log(`Fetch completed in ${duration}ms, status: ${response.status}`);
        ...
        console.error(`Detailed fetch error for ${url}:`, errorInfo);

        // Re-throw with more context instead of returning null
        throw new Error(`Failed to fetch ${url}: ${error.name} - ${error.message}`);
    }
}
...
export async function scrapeFleet() {
    ...
    const mainPageUrl = 'https://advantage-aviation.com/rental-aircraft/';
